...
# When the terminal strips markdown fences, code arrives as plain text.
# These patterns detect code-like content so we can re-wrap it in fences
# before format_html processes it.

# First-line patterns that strongly indicate code
_CODE_FIRST_LINE_RE = re.compile(
    r"^(?:"
    r"def\s"               # Python function
    r"|class\s"            # Python/JS/Java class
    r"|import\s"           # Python/JS/Java import
    r"|from\s\S+\simport"  # Python from-import
    r"|async\s+def\s"      # Python async function
    r"|function\s"         # JavaScript function
    r"|const\s"            # JavaScript const
    r"|let\s"              # JavaScript let
    r"|var\s"              # JavaScript var
    r"|export\s"           # JavaScript/TypeScript export
    r"|#!/"                # Shebang
    r"|package\s"          # Go/Java package
    r"|pub\s+fn\s"         # Rust pub fn
    r"|fn\s"               # Rust fn
    r"|struct\s"           # Rust/Go struct
    r"|enum\s"             # Rust/Java enum
    r"|interface\s"        # TypeScript/Java interface
    r"|type\s\w+\s*="      # TypeScript type alias
    r"|@\w+"               # Decorator (Python/Java)
    r"|module\s"           # Various languages
    r"|public\s"           # Java/C#
    r"|private\s"          # Java/C#
...
def wrap_code_blocks(text: str) -> str:
    """Detect code-looking content and wrap it in markdown code fences.

    The terminal strips markdown formatting, so code blocks from Claude
    arrive as plain text.  This function detects when the entire content
    block looks like source code (based on the first line matching common
    language keywords) and wraps it in triple-backtick fences so that
    :func:`reflow_text` preserves it and :func:`format_html` renders it
    as ``<pre><code>``.

    Only wraps when the content is *entirely* code — mixed prose-and-code
    is left alone to avoid false positives.

    Args:
        text: Extracted content text (before reflow).

    Returns:
        Text with code fences added if code was detected, otherwise
        unchanged.
    """
    if not text:
        return text
